...
const UI: Record<Locale, Record<string, string>> = {
  en: {
    "menu.subtitle": "Year campaign / Day 1",
    ...
    "menu.tutorialTime": "3-5 minutes",
    "menu.tutorialDescription": "Learn the basic loop with a guided board, then jump straight into a normal campaign.",
    "menu.startTutorial": "Start tutorial",
    "menu.skipTutorial": "Skip and start run",
    ...
    "header.calendar": "Week {week} · Day {dayInWeek}/{daysPerWeek} · Month {month} · Q{quarter}",
    "header.campaignProgress": "Year day {day}/{daysPerYear}",
    "header.goal": "Goal {value}/{goal} · Trust {trust}/{trustGoal}",
    "header.horizonGoal": "{horizon} {id}: Value {value}/{goal} · Trust {trust}/{trustGoal}",
    "header.goalMet": "Goal met",
    "header.horizonReviewTomorrow": "{horizon} review tomorrow morning",
    "header.horizonReviewInDays": "{horizon} review in {days} days",
    ...
    "horizonReview.title": "{horizon} {id} Review",
    "horizonReview.met": "Goals met",
    "horizonReview.missed": "Goals missed",
    "horizonReview.valueMet": "Value {actual}/{target} met",
    "horizonReview.valueMissed": "Value {actual}/{target} missed",
    "horizonReview.trustMet": "Trust {actual}/{target} met",
    "horizonReview.trustMissed": "Trust {actual}/{target} missed",
    "horizonReview.effect": "Goal effect",
    "horizonReview.cap": "Trust hit capped {value}/{cap}",
    ...
    "taskStory.stakes": "Why it matters",
    "taskStory.failurePreview": "If it fails",
    "subtasks.title": "Subtasks",
    "comments.title": "Task notes",
    ...
    "tutorial.stage.multi-work": "Several specialists",
    "tutorial.stage.compromise": "Compromise under pressure",
    "tutorial.stage.deadline-choice": "Deadline choice",
    "tutorial.stage.day-end": "End of day",
    ...
    "tutorial.step.move-multi-task": "Move the new task into In Progress",
    "tutorial.step.assign-backend": "Drag backend onto the task",
    "tutorial.step.wait-backend": "Wait for backend work",
    "tutorial.step.assign-frontend": "Drag frontend onto the task",
    "tutorial.step.wait-frontend": "Wait for frontend work",
    "tutorial.step.assign-qa-multi": "Drag QA onto the finished task",
    "tutorial.step.wait-qa-multi": "Wait for QA",
    "tutorial.step.move-multi-done": "Move the clean task to Done",
    "tutorial.step.move-compromise-task": "Move the urgent task into In Progress",
    "tutorial.step.assign-backend-compromise": "Drag backend onto the urgent task",
    "tutorial.step.wait-compromise-work": "Wait for the urgent fix",
    "tutorial.step.move-risky-done": "Ship without QA because QA is exhausted",
    "tutorial.step.move-deadline-task": "Move the deadline task into In Progress",
    "tutorial.step.assign-sre-deadline": "Drag SRE onto the task",
    "tutorial.step.wait-deadline-work": "Wait for SRE work",
    "tutorial.step.choose-deadline-outcome": "Choose: ship late or wait and miss it",
    "tutorial.step.wait-day-end": "Wait for the daily release report",
    ...
    "tutorial.startCampaign": "Start new game",
  },
  ru: {
    ...
    "menu.tutorialRecommended": "Рекомендуем обучение",
    "menu.tutorialTime": "3-5 минут",
    "menu.tutorialDescription": "Пройди базовый цикл на управляемой доске, а потом сразу начни обычную кампанию.",
    "menu.startTutorial": "Пройти обучение",
    "menu.skipTutorial": "Пропустить и начать игру",
    ...
    "header.calendar": "Неделя {week} · День {dayInWeek}/{daysPerWeek} · Месяц {month} · Q{quarter}",
    "header.campaignProgress": "Год: день {day}/{daysPerYear}",
    "header.goal": "Value {value}/{goal} · Доверие {trust}/{trustGoal}",
    "header.horizonGoal": "{horizon} {id}: Value {value}/{goal} · Доверие {trust}/{trustGoal}",
    "header.goalMet": "Цель закрыта",
    "header.horizonReviewTomorrow": "Ревью: {horizon} завтра утром",
    "header.horizonReviewInDays": "Ревью: {horizon} через {days} дн.",
    ...
    "header.pause": "Пауза",
    "header.menu": "Меню",
    "header.timeScale": "Скорость времени",
    ...
    "horizonReview.title": "Ревью: {horizon} {id}",
    "horizonReview.met": "Цели закрыты",
    "horizonReview.missed": "Цели не закрыты",
    "horizonReview.valueMet": "Value {actual}/{target} закрыт",
    "horizonReview.valueMissed": "Value {actual}/{target} не закрыт",
    "horizonReview.trustMet": "Доверие {actual}/{target} закрыто",
    "horizonReview.trustMissed": "Доверие {actual}/{target} не закрыто",
    "horizonReview.effect": "Эффект цели",
    "horizonReview.cap": "Урон доверию ограничен {value}/{cap}",
    ...
    "taskStory.stakes": "Почему важно",
    "taskStory.failurePreview": "Если провалить",
    "subtasks.title": "Подзадачи",
    "comments.title": "Заметки задачи",
    ...
    "tutorial.stage.multi-work": "Несколько специалистов",
    "tutorial.stage.compromise": "Компромисс под давлением",
    "tutorial.stage.deadline-choice": "Выбор по дедлайну",
    "tutorial.stage.day-end": "Конец дня",
    ...
    "tutorial.step.move-multi-task": "Перенеси новую задачу в работу",
    "tutorial.step.assign-backend": "Перетащи бэкендера на задачу",
    "tutorial.step.wait-backend": "Дождись работы бэкенда",
    "tutorial.step.assign-frontend": "Перетащи фронтендера на задачу",
    "tutorial.step.wait-frontend": "Дождись работы фронта",
    "tutorial.step.assign-qa-multi": "Перетащи QA на готовую задачу",
    "tutorial.step.wait-qa-multi": "Дождись QA",
    "tutorial.step.move-multi-done": "Перенеси чистую задачу в Готово",
    "tutorial.step.move-compromise-task": "Перенеси срочную задачу в работу",
    "tutorial.step.assign-backend-compromise": "Перетащи бэкендера на срочную задачу",
    "tutorial.step.wait-compromise-work": "Дождись срочного фикса",
    "tutorial.step.move-risky-done": "Отправь без QA: QA выдохся",
    "tutorial.step.move-deadline-task": "Перенеси задачу с дедлайном в работу",
    "tutorial.step.assign-sre-deadline": "Перетащи SRE на задачу",
    "tutorial.step.wait-deadline-work": "Дождись работы SRE",
    "tutorial.step.choose-deadline-outcome": "Выбери: отправить поздно или ждать и сорвать",
    "tutorial.step.wait-day-end": "Дождись дневного отчета",
    ...
    "tutorial.startCampaign": "Начать новую игру",
  },
};
...
const EXACT_TEXTS: Record<Locale, Record<string, string>> = {
  ...
  ru: {
    ...
    "Add a small customer note": "Добавить небольшую заметку клиента",
    "Patch urgent login error": "Срочно починить ошибку логина",
    "Stabilize partner export": "Стабилизировать экспорт партнера",
    "Implement backend change": "Сделать бэкенд-изменение",
    "Update visible UI state": "Обновить видимое состояние UI",
    "Check the finished change": "Проверить готовое изменение",
    "Patch backend behavior": "Починить поведение бэкенда",
    "Verify the urgent fix": "Проверить срочный фикс",
    "Stabilize production path": "Стабилизировать production path",
...
const subtaskTitleMap = buildBidirectionalMap({
  en: [
    ...Object.values(SIM_TEXT.en.subtasks),
    "Implement backend change",
    "Update visible UI state",
    "Check the finished change",
    "Patch backend behavior",
    "Verify the urgent fix",
    "Stabilize production path",
  ],
  ru: [
    ...Object.values(SIM_TEXT.ru.subtasks),
    "Сделать бэкенд-изменение",
    "Обновить видимое состояние UI",
    "Проверить готовое изменение",
    "Починить поведение бэкенда",
    "Проверить срочный фикс",
    "Стабилизировать production path",
  ],
});
...
export function localizeText(raw: string | null | undefined, locale: Locale): string {
  ...
  const replacements: Array<[RegExp, (...groups: string[]) => string]> = [
    ...
    [/^QA coverage is partial \((?<actual>\d+)\/(?<target>\d+)\)\. Assign QA again to clear release risk\.$/, (_all, actual, target) => `QA-покрытие частичное (${actual}/${target}). Назначь QA еще раз, чтобы убрать риск релиза.`],
...
export function localizeEffect(raw: string, locale: Locale): string {
  ...
  const replacements: Array<[RegExp, (...groups: string[]) => string]> = [
    ...
    [/^horizon reviews (?<value>\d+)$/, (_all, value) => `ревью горизонтов ${value}`],
    [/^no horizon review$/, () => "нет ревью горизонта"],
    [/^qa coverage (?<actual>\d+)\/(?<target>\d+)$/, (_all, actual, target) => `QA-покрытие ${actual}/${target}`],
    [/^goal met$/, () => "цель закрыта"],
    [/^goal missed$/, () => "цель не закрыта"],
    [/^value (?<actual>\d+)\/(?<target>\d+)$/, (_all, actual, target) => `value ${actual}/${target}`],
    [/^trust (?<actual>\d+)\/(?<target>\d+)$/, (_all, actual, target) => `доверие ${actual}/${target}`],
    [/^raw trust damage (?<value>\d+)$/, (_all, value) => `сырой урон доверию ${value}`],
    [/^capped trust damage (?<value>\d+)$/, (_all, value) => `ограниченный урон доверию ${value}`],
    [/^today trust damage (?<value>.+)$/, (_all, value) => `урон доверию сегодня ${value}`],
...
export function localizeEventTitle(raw: string, locale: Locale): string {
  ...
  const replacements: Array<[RegExp, (...groups: string[]) => string]> = [
    ...
    [/^(?<id>[A-Z]+-\d+) QA coverage partial$/, (_all, id) => `${id}: частичное QA-покрытие`],
    ...
    [/^(?<kind>week|month|quarter|year) (?<id>\d+) review$/, (_all, kind, id) => `${t(locale, `horizon.${kind}`)} ${id}: ревью`],
...
export function localizeEventBody(raw: string, locale: Locale): string {
  ...
  const replacements: Array<[RegExp, (...groups: string[]) => string]> = [
    ...
    [/^(?<name>.+) added partial QA coverage\.$/, (_all, name) => `${name} добавил(а) частичное QA-покрытие.`],
    ...
    [/^Business goals were met\.$/, () => "Бизнес-цели закрыты."],
    [/^Business goals were missed\.$/, () => "Бизнес-цели не закрыты."],
    [/^(?<count>\d+) business horizon review\(s\) completed\.$/, (_all, count) => `Завершено ревью бизнес-горизонтов: ${count}.`],
  ];
